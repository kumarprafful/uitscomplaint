...
<body>
	<div class="container-fluid">
		<div class="content">
			<input type="text" id="myInput" onkeyup="filterTable()" placeholder="Search For Username/Designation.." style="width: 30%;">
				
		<?php 
			$userdata = array();
			//$connect = mysqli_connect('localhost','root','','comp_sys');
			$query = mysqli_query($connect, "SELECT * FROM technician");
			...
			?>
				...
				<?php 
					...
					foreach ($userdata as $user) {
						if($user['designation'] != "superuser"){
						?>
						<tr>
							<td><?php echo $i;$i++.'<br>'; ?></td>
							<td><?php echo $user['tech_id'].'<br>'; ?></td>
							<td><?php echo $user['username'].'<br>'; ?></td>
							<td><?php echo $user['first_name'].' '.$user['last_name'].'<br>'; ?></td>
							<td><?php echo $user['email'].'<br>'; ?></td>
							<td><?php echo $user['designation'].'<br>'; ?></td>
						
						<!-- <	<td id="status"><?php #if($user[active] == 1) {echo 'Active';} else{echo 'Inactive';} ?></td> -->
							<td><?php if($user['active'] == 1){ ?>
								<button class="btn btn-default" id="deactivebtn" value="<?php echo $user['tech_id'] ?>" onclick="deactivate(this)">Deactivate</button>
								<?php } 
								else {
									?>
								<button class="btn btn-default" id="activebtn" value="<?php echo $user['tech_id']?>" onclick="activate(this)">Activate</button>
...
function activate(el){
		...
		xmlhttp.open("GET","tech_activate.php?q="+el.value,true);
		xmlhttp.send();
	}
...
function deactivate(el){
		...
		xmlhttp.onreadystatechange = function() {
        	if (this.readyState == 4 && this.status == 200) {
				...
				el.innerHTML = "Deactivated";
				el.id = "deactivebtn";
        	}
   		};
		xmlhttp.open("GET","tech_deactivate.php?q="+el.value,true);
		xmlhttp.send();
	}
